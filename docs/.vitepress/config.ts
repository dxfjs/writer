...
export default defineConfig({
  ...
  markdown: {
    toc: { level: [2] },
    config(md) {
      // cast: vitepress and the plugin resolve different markdown-it type versions
  ...
  themeConfig: {
    ...
    sidebar: [
      ...
      {
        text: "Tutoriels",
        items: [{ text: "About Extended Data", link: "/tutoriels/xdata" }],
      },
    ],
    footer: {
  ...
  vite: {
    // cast: vitepress and the plugin resolve different vite type versions
    plugins: [
      groupIconVitePlugin() as unknown as NonNullable<
        NonNullable<import("vitepress").UserConfig["vite"]>["plugins"]
      >[number],
    ],
  },
});

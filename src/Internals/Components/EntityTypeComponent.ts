import Tag          from    "../Tag";
import DXFInterface from    "../Interfaces/DXFInterface";

export default class EntityTypeComponent implements DXFInterface
{
    get name() : string { return this._name.value.toString(); }

    set name(value : string) { this._name.value = value; }

    private readonly _name : Tag;

    public constructor(name : string) { this._name = new Tag(0, name); }

    stringify(): string { return this._name.stringify(); }

    tags(): Tag[] { return [this._name]; }
}
